...
namespace Org.BouncyCastle.Pkcs.Tests
{
	...
	*/
	...
	public class Pkcs12StoreTest
		: SimpleTest
	{
		private static readonly char[] passwd = { 'h', 'e', 'l', 'l', 'o', ' ', 'w', 'o', 'r', 'l', 'd' };
		...
		*/
		...
		public void doTestPkcs12Store()
		{
			...
			Pkcs12Store store = new Pkcs12Store(stream, passwd);
			...
			store = new Pkcs12Store(stream, passwd);
			...
			store = new Pkcs12Store(stream, "user".ToCharArray());
			...
			chain[0] = new X509CertificateEntry(CreateCert(pubKey, privKey));
			...
			{
				Fail("key not dentified as key entry");
			}
			...
			store.Save(new MemoryStream(), passwd, new SecureRandom());

			//
			// no friendly name test
			//
			stream = new MemoryStream(pkcs12noFriendly, false);
			store = new Pkcs12Store(stream, noFriendlyPassword);
			...
			store = new Pkcs12Store(stream, storagePassword);
			...
			store = new Pkcs12Store(stream, "".ToCharArray());
		}

		...
		: ITest
	{
		...
		public ITestResult Perform()
		{
			try
			{
				...
				{
					return new SimpleTestResult(false, "PSS Sign test expected "
...
//					new PSSParameterSpec("SHA-512", "MGF1", new MGF1ParameterSpec("SHA-512"), 0, 1));
				...
				// TODO Put How to do this via SignerUtilities/Init
//				s = new PssSigner(new RsaEngine(), new Sha512Digest(), 0, 1);
				// The test sig1 actually seems to have implicit trailer (ignores trailer=1 arg above?)
				s = new PssSigner(new RsaEngine(), new Sha512Digest(), 0);
				s.Init(true, privKey);
...
//				s = new PssSigner(new RsaEngine(), new Sha512Digest(), 0, 1);
			...
			{
				return new SimpleTestResult(false, Name + ": exception - " + e.Message, e);
			}
		}

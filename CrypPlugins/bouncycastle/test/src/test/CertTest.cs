...
//			MemoryStream bIn = new MemoryStream(cert.GetEncoded(), false);
...
//			bIn = new MemoryStream(cert.GetEncoded());
...
//				CRL cert = fact.generateCRL(bIn);
		...
		private void checkCrlCreation1()
		{
			...
			if (!crl.IssuerDN.Equivalent(new X509Name("CN=Test CA")))
			{
...
//			Certificate cert = cf.generateCertificate(new ByteArrayInputStream(PemData.CERTIFICATE_1.getBytes("US-ASCII")));
			X509Certificate cert = new X509CertificateParser().ReadCertificate(Encoding.ASCII.GetBytes(PemData.CERTIFICATE_1));
			if (cert == null)
			{
				Fail("PEM cert not read");
			}
//			CRL crl = cf.generateCRL(new ByteArrayInputStream(Encoding.ASCII.GetBytes(PemData.CRL_1));
			X509Crl crl = new X509CrlParser().ReadCrl(Encoding.ASCII.GetBytes(PemData.CRL_1));
			if (crl == null)
			{
				Fail("PEM crl not read");
			}
//			ICollection col = cf.generateCertificates(new ByteArrayInputStream(PemData.CERTIFICATE_2.getBytes("US-ASCII")));

...
namespace Cryptool.Playfair
{
    ...
    [PluginInfo(false, "Playfair", "Playfair -- classic polygraphic substitution cipher", "", "Playfair/icon.png", "Playfair/Images/encrypt.png", "Playfair/Images/decrypt.png")]
        ...
        private string preFormatText()
        {
            ...
            if (settings.SeperatePairs)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    {
                        if (s[i] == settings.Separator)
                            s = s.Insert(i+1, settings.SeparatorReplacement.ToString());
                        else
                            s = s.Insert(i+1, settings.Separator.ToString());
                        length++;
                    }
                }
            }

            //if a char doesn't have a second pair char
            if (s.Length % 2 != 0)

...
namespace Be.Windows.Forms
{
    ...
    private delegate void vScrollDelegate();
        ...
        vScrollDelegate vScrollDel = delegate()
        {
          _vScrollBar.Value = ToScrollPos(_scrollVpos);
          _vScrollBar.Enabled = true;
        };
        _vScrollBar.Invoke(vScrollDel);
			}
			else
			{
        ...
        if (pos % 20000 == 0) // for performance reasons: DoEvents only 1 times per 1000 loops
        {
          //Application.DoEvents();
          if (this.OnStatusBarProgressbarValueChanged != null)
          {
            double current = Math.Ceiling((double)_findingPos / _byteProvider.Length * 100);
            OnStatusBarProgressbarValueChanged.BeginInvoke(null, new PluginProgressEventArgs(current, 100), null, null);
            //OnStatusBarProgressbarValueChanged(
            //  this, new StatusBarProgressbarValueChangedEventArgs(current, 100));
          }
        }
          ...
          //if (OnFindFinished != null) OnFindFinished(this);
    ...
    public void AbortFind()
    {
      //if (OnFindFinished != null) OnFindFinished(this);
      _abortFind = true;
      // while (_abortFind) System.Threading.Thread.Sleep(10);
    }

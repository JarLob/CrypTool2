...
// $Revision:: 38                                                                             $://
// $Author:: junker                                                                           $://
// $Date:: 2008-11-20 17:32:42 +0100 (Do, 20 Nov 2008)                                        $://
//////////////////////////////////////////////////////////////////////////////////////////////////
...
namespace Whirlpool
{
	...
	public class WhirlpoolHash
	{
		...
		void processBuffer()
		{
			...
			{
				string tmp = string.Format("    {0,2:X} {1,2:X} {2,2:X} {3,2:X} {4,2:X} {5,2:X} {6,2:X} {7,2:X}\n",
						buffer[ibuffer], buffer[ibuffer + 1], buffer[ibuffer + 2], buffer[ibuffer + 3],
						buffer[ibuffer + 4], buffer[ibuffer + 5], buffer[ibuffer + 6], buffer[ibuffer + 7]);
				Debug.WriteLine(tmp);
			}
			Debug.WriteLine("\n");
#endif
			...
			for (int i = 0; i < DIGESTBYTES / 8; i++)
			{
				string tmp = string.Format("    {0,16:X} {1,16:X}", K[i], state[i]);
				Debug.WriteLine(tmp);
			}
			Debug.WriteLine("");
   ...
   */
			for (r = 1; r <= R; r++)
			{
				...
				for (int i = 0; i < DIGESTBYTES / 8; i++)
				{
					string tmp = string.Format("    {0,16:X} {1,16:X}\n", K[i], state[i]);
					Debug.WriteLine(tmp);
				}
				Debug.WriteLine("\n");
#endif
			...
			for (int i = 0; i < DIGESTBYTES / 8; i++)
			{
				string tmp = string.Format("    {0,16:X}", hash[i]);
				Debug.WriteLine(tmp);
			}
			Debug.WriteLine("");
#endif

...
namespace CertificateLibrary.Certificates
{
    public static class CertificateServices
    {
            ...
            for (int i = 0; i < rgFiles.Length; i++)
            {
                if (File.Exists(String.Format("{0}{1}.p12", filepath, Path.GetFileNameWithoutExtension(rgFiles[i].Name))))
                {
                    ...
                    try
                    {
                        ...
                        Org.BouncyCastle.X509.X509CertificateParser x509Parser = new X509CertificateParser();
        ...
        public static List<X509Certificate> GetX509Certificates(string filepath)
        {
            DirectoryInfo di = new DirectoryInfo(filepath);
            FileInfo[] rgFiles = di.GetFiles("*.crt");
        ...
        public static Org.BouncyCastle.X509.X509Certificate GetX509CertificateByEmail(string filepath, string email)
        {
            ...
            foreach (Org.BouncyCastle.X509.X509Certificate cert in GetX509Certificates(filepath))
            {
                if (SHA1.ComputeHashString(email.ToLower()).Equals(GetExtensionValue(cert, PAPObjectIdentifier.HashedEmail)))
                {
                    return cert;
                }
            }
        ...
        public static string GetExtensionValue(Org.BouncyCastle.X509.X509Certificate certificate, string oid)
        {
            ...
            // This may look complicated, but BouncyCastles DerUtf8String only gives a dirty string :(
            Org.BouncyCastle.Asn1.DerUtf8String value = Org.BouncyCastle.Asn1.DerUtf8String.GetInstance(certificate.GetExtensionValue(new Org.BouncyCastle.Asn1.DerObjectIdentifier(oid)));
            if (value == null)
            {
                return null;
            }

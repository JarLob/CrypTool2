...
{
    internal class PropagateStateHandler : AMessagesHandler<PropagateStateMessage>
    {
        public PropagateStateHandler(IManagementLayerCallback managementCallback, CommunicationLayer commLayer, CertificateService certificateService)
            : base(managementCallback, commLayer, certificateService) {}

        public override void HandleValidPacket(PropagateStateMessage message, IPAddress from)
        {
            managementCallback.OnIncomingState(message, from);
